...
function pinboard_available_fonts() {
	return array(
		...
		'Yahei'  => '"Microsoft Yahei","Luxi Sans", "DejaVu Sans","Hiragino Sans GB"',
...
function pinboard_register_styles() {
	wp_register_style( 'slidebars', get_template_directory_uri() . '/styles/slidebars.css', false, null );
	$web_fonts = array(
...
function pinboard_register_scripts() {
	...
	wp_register_script( 'slidebarsmini', get_template_directory_uri() . '/scripts/slidebars.min.js', array( 'jquery' ), null );
}
endif;
...
function pinboard_enqueue_scripts() {
	...
	wp_enqueue_script( 'slidebarsmini' );
...
<style>
.top_tags:hover {width:100px;border: 1px solid #427FED;}
</style>
	<![endif]-->
<?php
}
...
$('#menu-triggle').click(function(){
	if($('#guide').css("display")=='none'){
		$('#guide').css({"display":"block","z-index":10});
	}else{
		$('#guide').css({"display":"none","z-index":0});
	}
});
$('.guide-module-toggle').click(function(){
	$(this).toggleClass('collapsed');
	var a=function(){$('.subscription-container').css("display",function(index,value){return value=='none'?'block':'none';})};
	setTimeout(a,200);
})
	...
	if ( ! ( is_home() && ! is_paged() ) && ! is_singular() || isset( $pinboard_page_template ) ) {
		...
		<hgroup id="current-location">
			...
			<<?php pinboard_title_tag( 'location' ); ?> class="page-title">
				...
				?>
				<div><?php echo get_avatar( $author->ID, 120 );?></div>					
				<div class="userinfo">
				<span class="displayname"><?php echo $author->display_name; ?></span>
				</div>	
				<?php
				} elseif ( is_year() ) {
...
function pinboard_content_class( $classes = array() ) {
	$classes[] = 'column';
	/*
	if( is_author() || is_category()||is_category( pinboard_get_option( 'portfolio_cat' ) )||(is_home()&& 'full-width' == pinboard_get_option( 'layout' ) )|| 'no-sidebars' == pinboard_get_option( 'layout' ) )
*/	
	if( !is_single())
		$classes[] = 'onecol';
	...
	if( true||! pinboard_is_teaser() ) : ?>
		<aside class="entry-meta">
			...
			<?php else : ?>
				...
				<span class="entry-author-link">来源：<?php the_author_posts_link(); ?></span>
				<span class="entry-permalink"><a href="<?php echo get_post_meta(get_the_ID(),'original_url',true); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>" target="_blank">阅读原文</a></span>
				<?php endif; ?>
				<?php if( ! is_attachment() ) : ?>
				<span class="entry-category"><?php the_category( ', ' ); ?></span>
				<?php elseif( wp_attachment_is_image() ) : ?>
					<span class="attachment-size"><a href="<?php echo wp_get_attachment_url(); ?>" title="<?php _e( 'Link to full-size image', 'pinboard' ); ?>"><?php $metadata = wp_get_attachment_metadata();if($metadata){echo $metadata['width']; }?> &times; <?php if($metadata){echo $metadata['height'];} ?></a> <?php _e( 'pixels', 'pinboard' ); ?></span>
				<?php endif; ?>
				<?php if( is_singular() ) : ?>
...
_delete_post_link("删除",'<span class="edit-link">','</span>');

?>

				<?php if( ! is_singular() ) : ?>
					<span class="entry-permalink"><a href="<?php echo get_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>" target="_blank">全文</a></span>
				<?php endif; ?>
			<?php endif; ?>
			<div class="clear"></div>
		</aside><!-- .entry-meta -->
...
if ( ! function_exists( '_delete_post_link' ) ) :
# DELETE POST LINK
...
function chinese_excerpt($text,$lenth=200){
...
function pinboard_post_image() {
	if( has_post_thumbnail()) : ?>
		<figure>
			<a href="<?php /*$image = wp_get_attachment_image_src( get_post_thumbnail_id(), 'full' ); echo $image[0]*/ the_permalink(); ?>" title="<?php the_title_attribute(); ?>" class="colorbox" rel="attachment" target="_blank">
				<?php the_post_thumbnail( ( pinboard_is_teaser() ? 'teaser-thumb' : 'image-thumb' ) ); ?>
			</a>
		</figure>
	<?php else :
		// Retrieve the FIRST image attached to the post
		$args = array(
			'numberposts' => 1,
		...
		if( count( $attachments ) ) {
			...
			if( isset( $attachment ) && ! post_password_required() ) :
				...
				if($image[1]>100&& $image[2]>100): /*width>0  make sure the image ize is larger than 100*100*/?>
				<figure>
					<a href="<?php the_permalink();/*echo $image[0];*/ ?>" title="<?php the_title_attribute(); ?>" class="colorbox"  rel="attachment" target="_blank">
						<?php echo wp_get_attachment_image( $attachment->ID, 'teaser-thumb'/*'image-thumb'*/ ); ?>
					</a>
				</figure>
				<?php endif;
			endif;
		...
		} else {//comment out, I dont need content in this place. 2012-10-21 22:30 luchunwei
			//the_content();
		}
	endif;
}
...
if ( ! function_exists( 'imdevice_get_first_image_url' ) ) :
function imdevice_get_first_image_url($content) {
  preg_match_all('/<img[^>]+src=[\'"]([^\'"]+)[\'"].*>/isU', $content, $image);
  $p_sum = count($image[1]);
  if ($p_sum > 0) {
    $url = array("image", $image[1][0]);
  } 
//echo $url;echo $url[0];echo $url[1];
  return $url[1];
}
endif;

if ( ! function_exists( 'imdevice_get_first_image' ) ) :
function imdevice_get_first_image($echo=true) {
$first_image_url=get_post_meta(get_the_ID(),'original_first_image_url',true);
if(empty($first_image_url)){
	$content = apply_filters( 'the_content', get_the_content( '', true ) );
	if( '' != $content ) {
		$first_image_url=imdevice_get_first_image_url($content);
	}
	if($first_image_url){update_post_meta(get_the_ID(),'original_first_image_url',$first_image_url);}
}
if(!$echo) return $first_image_url;
if($first_image_url){
	printf(	'<figure><img src="%s" alt="%s"/></figure>',$first_image_url, the_title_attribute( 'echo=0' ));
}
}
endif;

if ( ! function_exists( 'imdevice_post_image' ) ) :
/**
 * Show the last image attached to the current post
 *
 * Used in image post formats
	...
	if ( $wp_query->max_num_pages > 1 ) {
		switch( pinboard_get_option( 'posts_nav_labels' ) ) {
			case 'next/prev' :
				...
				break;
			case 'older/newer' :
				$prev_label = '新近文章'/*__( 'Newer Posts', 'pinboard' )*/;
				$next_label ='更早文章'/* __( 'Older Posts', 'pinboard' )*/;
				break;
			case 'earlier/later' :
		...
		<?php else : ?>
			<div id="posts-nav" class="navigation">
				...
				<?php if( is_home() && ! is_paged() ) : ?>
					<div class="nav-all"><?php next_posts_link( __( 'Read More Articles', 'pinboard' ) . '&rarr;' ); ?></div>
				<?php else : ?>
...
function pinboard_404() { ?>
	<article class="post hentry column onecol" id="post-0">
		<h2 class="entry-title"><?php _e( '内容没找到', 'pinboard' ) ?></h2>
		<div class="entry-content">
			<?php _e( '你要的内容没找到或者暂时没准备就绪，请稍后再试，返回首页', 'pinboard' ); ?></p>
...
if ( ! function_exists( 'imdevice_entry_meta' ) ) :
/**
 * Output entry-meta tag
 *
 * @since Imdevice 0.1
 */
function imdevice_entry_meta() {

	?>
	<aside class="entry-meta-social">
		<!--<span class="entry-date"><?php the_time( get_option( 'date_format' ).' '.get_option( 'time_format' ) ); ?></span>-->
		<?php if(function_exists(myLikeButton())){myLikeButton();} ?>
		<!--<div class="kklike-content kk-right kklike-button-big-light entry-comments-number">
		<div class="ico-comments-number2"></div>
		<a href="<?php echo get_permalink(); ?>#uyan_frame" class="kklike-value" id="uyan_count_unit" title="发表评论" >0 评论</a>
		</div>-->
		<div class="kklike-content kk-right kklike-button-big-light">
		<a href="<?php echo get_permalink(); ?>#uyan_frame" class="entry-comments-number" title="发表评论" >		
			<div class="ico-comments-number2">
			</div>
			<div class="kklike-value" ><?php comments_number('+','1','%'); ?></div>
			<div class="kklike-text">评论</div>
		</a></div>
		
		<!--
		<span class="entry-comments-number"><?php comments_number('评论','1 评论','% 评论'); ?></span>
		<span class="entry-liked"><?php if(function_exists(kkLikeRating())){kkLikeRating();} ?>喜欢</span>
		-->
		<div class="clear"></div>
	</aside><!-- .entry-meta -->
	<?php 
...
if ( ! function_exists( 'past_date' ) ) :
/**
 * Display Data Time as  时间显示xx前
 *
 * @since Imdevice 0.1
 */
//时间显示xx前
function past_date(){
	...
	if($past_time < $m){ //小于1分钟
			...
			echo get_post_time(get_option( 'date_format' ) .' '.get_option( 'time_format' ));
			return;
	}
...
add_filter('the_time', 'time_since');
endif;

if ( ! function_exists( 'my_avatar' ) ) :
/**
 * 缓存Avatar为本地头像
 *
 * @since Imdevice 0.1
...
function my_avatar($avatar) {
  $local='wp-content/uploads/avatar/';
  $tmp = strpos($avatar, 'http');
  $g = substr($avatar, $tmp, strpos($avatar, "'", $tmp) - $tmp);
  $tmp = strpos($g, 'avatar/') + 7;
  $f = substr($g, $tmp, strpos($g, "?", $tmp) - $tmp);
  $w = site_url();//get_bloginfo('wpurl');
  $e = ABSPATH .$local. $f .'.png';
  $t = 1209600; //设定14天, 单位:秒
  if ( !is_file($e) || (time() - filemtime($e)) > $t ) { //单头像不存在或文件超过设定天数时才更新
    copy(htmlspecialchars_decode($g), $e);
  } else  $avatar = strtr($avatar, array($g => $w.'/'.$local.$f.'.png'));
  if (filesize($e) < 500) copy($w.'/'.$local.'default.png', $e);
  return $avatar;
}
//add_filter('get_avatar', 'my_avatar');
endif;

if ( ! function_exists( 'baidu_share' ) ) :
/**
 * 百度分享
 *
 * @since Imdevice 0.1
 */
function baidu_share($content) {
if(is_single()){
//<!-- Baidu Button BEGIN -->
$baidu='
...
<script type="text/javascript" id="bdshare_js" data="type=tools&amp;uid=6487130" ></script>
...
//add_action( 'the_content', 'baidu_share', 15);
endif;

add_filter('wp_mail_from','mail_from');
function mail_from() {
$emailaddress = 'info@imdevice.com';
return $emailaddress;
}
...
function showAD(){//show Baidu Ads on home page
?>
<article class="post-001 post type-post status-publish format-image hentry category-news column threecol masonry-brick"><div class="entry">
<div style="position:relative;max-width:343px;max-height:284px;margin:0 auto;text-align:center;">
<script type="text/javascript">
/*336*280，创建于2013-5-30*/
var cpro_id = "u1294532";
</script>
...
/** placehold for Google Custom Search**/
if ( ! function_exists( 'showGoogleCS' ) ) :
function showGoogleCS(){
?>
<article class="post-001 post type-post status-publish format-image hentry category-news column threecol masonry-brick"><div class="entry">
<div style="position:relative;max-width:343px;min-height:80px;margin:0 auto;">

<script>
  (function() {
    var cx = '014863106134628398529:mhnqezp2agg';
...
/**判断是否来自新浪微博爱问转码**/
if ( ! function_exists( 'is_from_weibo' ) ) :
function is_from_weibo(){
	$useragent=$_SERVER['HTTP_USER_AGENT'];
	if (stristr($useragent, 'SINA_WEIBO')) {
		return true;
	}
	return false;
}
...
function imdevice_remove_html_tags($str) {
...
if ( ! function_exists( 'add_first_image_to_feed' ) ) :
function add_first_image_to_feed($content) {
if(is_feed()) {
	$url=imdevice_get_first_image_url($content);
	$content=imdevice_remove_html_tags($content);
	$content = mb_substr($content,0, 200);	
	$content .='... <a href="'.get_permalink().'" rel="bookmark" title="'.the_title_attribute( 'echo=0' ).'" target="_blank">阅读全文</a>';
	if($url){
		$alt=the_title_attribute( 'echo=0' );
		$content='<a href="'.get_permalink().'"><img src="'.$url.'" alt="'.$alt.'"/></a><br>'.$content;
	}
	
}//end if is_feed
return $content;
}
add_filter ('the_content', 'add_first_image_to_feed');
endif;

if ( ! function_exists( 'wpdaxue_add_contact_fields' ) ) :
/**
 * 自定义用户个人资料信息
...
class baiduAdWidget extends WP_Widget {
function baiduAdWidget(){
	parent::WP_Widget(false, $name = '广告');   
}
/*输出工具内容*/
function widget($args, $instance) { 
	echo '<aside id="text-9" class="widget widget_text"><div class="textwidget">';
	if(is_mobile()){
	echo '<script type="text/javascript">
/*20:5，创建于2014-2-28*/
var cpro_id = "u1430352";
</script>
<script src="http://cpro.baidustatic.com/cpro/ui/cm.js" type="text/javascript"></script>';
	}else{
	echo '<script type="text/javascript">
/*300*250，创建于2013-7-31*/
var cpro_id = "u1331452";
</script>
<script src="http://cpro.baidustatic.com/cpro/ui/c.js" type="text/javascript"></script>

<script type="text/javascript">
/*300*250，创建于2014-1-23*/
var cpro_id = "u1460572";
</script>
